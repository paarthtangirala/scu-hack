...
EXISTING_LIVE_FRAME_INTERVAL_MS=""
...
if [[ -f "${MOBILE_ENV_FILE}" ]]; then
  ...
  EXISTING_LIVE_FRAME_INTERVAL_MS="$(grep -E '^EXPO_PUBLIC_LIVE_PREVIEW_FRAME_INTERVAL_MS=' "${MOBILE_ENV_FILE}" | head -n 1 | cut -d '=' -f 2- || true)"
fi
...
if [[ -n "${EXISTING_LIVE_FRAME_INTERVAL_MS}" ]]; then
  echo "EXPO_PUBLIC_LIVE_PREVIEW_FRAME_INTERVAL_MS=${EXISTING_LIVE_FRAME_INTERVAL_MS}" >> "${MOBILE_ENV_FILE}"
fi
